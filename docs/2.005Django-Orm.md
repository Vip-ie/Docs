...
**创建一个项目，并在里面新建blog的app**

1. 在系统用户目录下创建一个存放博客项目文件名为`mysite`
2. 创建存放项目文件方法:`mkdir ~/mysite`
3. 创建python项目Django项目环境并命名为`mysite`
4. 创建python虚拟环境方法:`mkvirtualenv mysite`
5. 启动python虚拟环境方法:`workon mysite`
6. 安装Django方法:`pip install django`
7. 安装数据库驱动方法:`pip install pymysql`
8. 安装数据库显示方法:`pip install Pillow`


设置setting文件中的
1. ALLOWED_HOSTS:`ALLOWED_HOSTS = ['*']`
2. INSTALLED_APP:`INSTALLED_APPS = ['blog',]`
3. TEMPLATES:`TEMPLATES = [{'DIRS': [os.path.join(BASE_DIR, 'templates')],}]`
4. PyMySQL连接器配置:
```
#在主目录下的的__init__.py文件添加下面两句
import pymysql
